...
def loadDataset():
    ...
    dataFrame = yf.download('AAPL', start = '2010-01-01', end='2017-01-11')
    ...
    dfcomp = yf.download(['AAPL', 'GE', 'GOOG', 'IBM', 'MSFT'], start='2010-01-01', end='2017-01-11')['Adj Close']
...
def distKNN():
    ...
    # KNN Regression
    clfknndist = KNeighborsRegressor(n_neighbors=3,weights='distance')
    clfknndist.fit(X_train, y_train)
    distknn = clfknndist.score(X_train, y_train)

    text.insert(END, "Accuracy of KNN with Distinct weights : "+str(distknn*100)+"\n\n")
def predModel():
    text.delete('1.0', END)
